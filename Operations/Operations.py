...
class Operations(object):
    ...
    def __init__(self):
        ...
        ## Load forecasting data when electricity_price_forecasting is enabled
        if electricity_price_forecasting:
            self._forecast_df = self._load_forecast_file()
            self._targets_df = self._load_targets_file()
    ...
    def _calculate_project_operations_df(self):
        ...
        for year_count in range(self._number_years_to_calculate):
            ...
            ## When forecasting, build settlement df from targets (actual prices for cashflow)
            if electricity_price_forecasting:
                settlement_price_period_df = self._build_settlement_price_df(electricity_price_period_df)
            else:
                settlement_price_period_df = electricity_price_period_df

            annual_operations_df = self._calculate_annual_operation_df(electricity_price_period_df, resource_input_flow_period_df, weather_data_period_df, rss_start, rps_efficiency, rcs_efficiency,rps_efficiency_bypass, rcs_efficiency_bypass, settlement_price_df=settlement_price_period_df, year_count=year_count)
    ...
    def _calculate_annual_operation_df(self, electricity_price_df, resource_input_flow_df, weather_data_df, rss_year_day_0, rps_efficiency, rcs_efficiency,rps_efficiency_bypass, rcs_efficiency_bypass, settlement_price_df=None, year_count=0):
        ...
        ## for day_count in range(365):
            ...
            if electricity_price_forecasting:
                sample_period_df = self._get_forecast_sample_period_df(electricity_price_df, start, end, year_count, day_count)
            else:
        ...
        ## Use settlement_price_df for cashflow calculation (actual prices); fall back to electricity_price_df if not provided
        _settlement_df = settlement_price_df if settlement_price_df is not None else electricity_price_df
        annual_operations_df = self._calculate_aggregated_operations_df(annual_operation_df.copy(), annual_consolidated_resource_inlet_flow_list, annual_consolidated_resource_inlet_power_consumption_list, weather_data_df, rss_year_day_0).join(_settlement_df)
    ...
    def _load_forecast_file(self):
        """Load the DA forecast CSV and parse window_start as date objects."""
        path = os.path.join("MainData", inputs.forecast_file)
        df = pd.read_csv(path)
    ...
    def _load_targets_file(self):
        """Load the targets CSV and extract Year/Month/Day/Hour columns."""
        path = os.path.join("MainData", inputs.targets_file)
        df = pd.read_csv(path)
    ...
    def _build_settlement_price_df(self, period_df):
        """
        Build a settlement price DataFrame for cashflow calculation.
        Overlays actual prices from the targets file (settlement_column) onto the
        base electricity price period DataFrame.  Rows with no matching target entry
        keep the original (actual) prices from the base df as fallback.
        """
        settlement_df = period_df.copy()
        targets = self._targets_df[['Year', 'Month', 'Day', 'Hour', inputs.settlement_column]].copy()

        # Left-join to find actual settlement price for each period row
        merged = period_df[['Year', 'Month', 'Day', 'Hour']].merge(
            targets,
            on=['Year', 'Month', 'Day', 'Hour'],
            how='left'
        )
        actual_prices = merged[inputs.settlement_column]
        has_actual = actual_prices.notna().values

        if has_actual.any():
            actual_vals = actual_prices.values
            # Spot_Price2 = raw actual DA price (selling settlement)
            settlement_df.loc[has_actual, 'Spot_Price2'] = actual_vals[has_actual]
            # Spot_Price1 = actual price + energy tariffs + charges + margin, with electricity tax
            settlement_df.loc[has_actual, 'Spot_Price1'] = (
                actual_vals[has_actual]
                + settlement_df.loc[has_actual, 'EnergyTermTolls'].values * 1000
                + settlement_df.loc[has_actual, 'EnergyTermOtherCharges'].values * 1000
                + inputs.margin_tradingcompany_energy * 1000
            ) * (1 + inputs.electricity_tax / 100)

        return settlement_df

    def _get_forecast_sample_period_df(self, electricity_price_df, start, end, year_count, day_count):
        """
        Build the sample_period_df for trading decisions using forecast prices.
        Spot_Price1 (buy) and Spot_Price2 (sell) are replaced with values from the
        forecast file (inputs.forecast_column), with tariffs re-applied to Spot_Price1.
        Falls back to actual prices if no forecast window is found for the date.
        """
        ...
        # Determine the calendar date corresponding to this day_count
        calendar_date = date(self._project_year_start + year_count, 1, 1) + timedelta(days=day_count)

        forecast_window = self._forecast_df[self._forecast_df['window_start'] == calendar_date]
        n_hours = sample_period_days * 24
        n = min(len(base_slice), n_hours)

        if len(forecast_window) >= n:
            forecast_vals = forecast_window[inputs.forecast_column].values[:n]
            base_slice.iloc[:n, base_slice.columns.get_loc('Spot_Price2')] = forecast_vals
            base_slice.iloc[:n, base_slice.columns.get_loc('Spot_Price1')] = (
                forecast_vals
                + base_slice.iloc[:n]['EnergyTermTolls'].values * 1000
                + base_slice.iloc[:n]['EnergyTermOtherCharges'].values * 1000
                + inputs.margin_tradingcompany_energy * 1000
            ) * (1 + inputs.electricity_tax / 100)
        else:
            print(f"Warning: No forecast found for {calendar_date} (year_count={year_count}, day_count={day_count}). Using actual prices for trading decisions.")
